
-- SERİOUS CLOTHİNG KANK Ok?
Config = Config or {}

Config.WomanPlayerModels = {
    'mp_f_freemode_01',
    --[[ 'a_f_m_beach_01',
    'a_f_m_bevhills_01',
    'a_f_m_bevhills_02',
    'a_f_m_bodybuild_01',
    'a_f_m_business_02',
    'a_f_m_downtown_01',
    'a_f_m_eastsa_01',
    'a_f_m_eastsa_02',
    'a_f_m_fatbla_01',
    'a_f_m_fatcult_01',
    'a_f_m_fatwhite_01',
    'a_f_m_ktown_01',
    'a_f_m_ktown_02',
    'a_f_m_prolhost_01',
    'a_f_m_salton_01',
    'a_f_m_skidrow_01',
    'a_f_m_soucentmc_01',
    'a_f_m_soucent_01',
    'a_f_m_soucent_02',
    'a_f_m_tourist_01',
    'a_f_m_trampbeac_01',
    'a_f_m_tramp_01',
    'a_f_o_genstreet_01',
    'a_f_o_indian_01',
    'a_f_o_ktown_01',
    'a_f_o_salton_01',
    'a_f_o_soucent_01',
    'a_f_o_soucent_02',
    'a_f_y_beach_01',
    'a_f_y_bevhills_01',
    'a_f_y_bevhills_02',
    'a_f_y_bevhills_03',
    'a_f_y_bevhills_04',
    'a_f_y_business_01',
    'a_f_y_business_02',
    'a_f_y_business_03',
    'a_f_y_business_04',
    'a_f_y_eastsa_01',
    'a_f_y_eastsa_02',
    'a_f_y_eastsa_03',
    'a_f_y_epsilon_01',
    'a_f_y_fitness_01',
    'a_f_y_fitness_02',
    'a_f_y_genhot_01',
    'a_f_y_golfer_01',
    'a_f_y_hiker_01',
    'a_f_y_hipster_01',
    'a_f_y_hipster_02',
    'a_f_y_hipster_03',
    'a_f_y_hipster_04',
    'a_f_y_indian_01',
    'a_f_y_juggalo_01',
    'a_f_y_runner_01',
    'a_f_y_rurmeth_01',
    'a_f_y_scdressy_01',
    'a_f_y_skater_01',
    'a_f_y_soucent_01',
    'a_f_y_soucent_02',
    'a_f_y_soucent_03',
    'a_f_y_tennis_01',
    'a_f_y_tourist_01',
    'a_f_y_tourist_02',
    'a_f_y_vinewood_01',
    'a_f_y_vinewood_02',
    'a_f_y_vinewood_03',
    'a_f_y_vinewood_04',
    'a_f_y_yoga_01',
    'g_f_y_ballas_01',
    'g_f_y_families_01',
    'g_f_y_lost_01',
    'g_f_y_vagos_01',
    'mp_f_deadhooker',
    'mp_f_freemode_01',
    'mp_f_misty_01',
    'mp_f_stripperlite',
    'mp_s_m_armoured_01',
    's_f_m_fembarber',
    's_f_m_maid_01',
    's_f_m_shop_high',
    's_f_m_sweatshop_01',
    's_f_y_airhostess_01',
    's_f_y_bartender_01',
    's_f_y_baywatch_01',
    's_f_y_cop_01',
    's_f_y_factory_01',
    's_f_y_hooker_01',
    's_f_y_hooker_02',
    's_f_y_hooker_03',
    's_f_y_migrant_01',
    's_f_y_movprem_01',
    'ig_kerrymcintosh',
    'ig_janet',
    'ig_jewelass',
    'ig_magenta',
    'ig_marnie',
    'ig_patricia',
    'ig_screen_writer',
    'ig_tanisha',
    'ig_tonya',
    'ig_tracydisanto',
    'u_f_m_corpse_01',
    'u_f_m_miranda',
    'u_f_m_promourn_01',
    'u_f_o_moviestar',
    'u_f_o_prolhost_01',
    'u_f_y_bikerchic',
    'u_f_y_comjane',
    'u_f_y_corpse_01',
    'u_f_y_corpse_02',
    'u_f_y_hotposh_01',
    'u_f_y_jewelass_01',
    'u_f_y_mistress',
    'u_f_y_poppymich',
    'u_f_y_princess',
    'u_f_y_spyactress',
    'ig_amandatownley',
    'ig_ashley',
    'ig_andreas',
    'ig_ballasog',
    -- 'ig_maryannn',
    'ig_maude',
    'ig_michelle',
    'ig_mrs_thornhill',
    'ig_natalia',
    's_f_y_scrubs_01',
    's_f_y_sheriff_01',
    's_f_y_shop_low',
    's_f_y_shop_mid',
    's_f_y_stripperlite',
    's_f_y_stripper_01',
    's_f_y_stripper_02',
    'ig_mrsphillips',
    'ig_mrs_thornhill',
    'ig_molly',
    'ig_natalia',
    's_f_y_sweatshop_01',
    'ig_paige',
    'a_f_y_femaleagent',
    'a_f_y_hippie_01' ]]
}
    
Config.ManPlayerModels = {
    'mp_m_freemode_01',
    --[[ 'ig_trafficwarden',
    'ig_bankman',
    'ig_barry',
    'ig_bestmen',
    'ig_beverly',
    'ig_car3guy1',
    'ig_car3guy2',
    'ig_casey',
    'ig_chef',
    'ig_chengsr',
    'ig_chrisformage',
    'ig_clay',
    'ig_claypain',
    'ig_cletus',
    'ig_dale',
    'ig_dreyfuss',
    'ig_fbisuit_01',
    'ig_floyd',
    'ig_groom',
    'ig_hao',
    'ig_hunter',
    'csb_prolsec',
    'ig_joeminuteman',
    'ig_josef',
    'ig_josh',
    'ig_lamardavis',
    'ig_lazlow',
    'ig_lestercrest',
    'ig_lifeinvad_01',
    'ig_lifeinvad_02',
    'ig_manuel',
    'ig_milton',
    'ig_mrk',
    'ig_nervousron',
    'ig_nigel',
    'ig_old_man1a',
    'ig_old_man2',
    'ig_oneil',
    'ig_orleans',
    'ig_ortega',
    'ig_paper',
    'ig_priest',
    'ig_prolsec_02',
    'ig_ramp_gang',
    'ig_ramp_hic',
    'ig_ramp_hipster',
    'ig_ramp_mex',
    'ig_roccopelosi',
    'ig_russiandrunk',
    'ig_siemonyetarian',
    'ig_solomon',
    'ig_stevehains',
    'ig_stretch',
    'ig_talina',
    'ig_taocheng',
    'ig_taostranslator',
    'ig_tenniscoach',
    'ig_terry',
    'ig_tomepsilon',
    'ig_tylerdix',
    'ig_wade',
    'ig_zimbor',
    's_m_m_paramedic_01',
    'a_m_m_afriamer_01',
    'a_m_m_beach_01',
    'a_m_m_beach_02',
    'a_m_m_bevhills_01',
    'a_m_m_bevhills_02',
    'a_m_m_business_01',
    'a_m_m_eastsa_01',
    'a_m_m_eastsa_02',
    'a_m_m_farmer_01',
    'a_m_m_fatlatin_01',
    'a_m_m_genfat_01',
    'a_m_m_genfat_02',
    'a_m_m_golfer_01',
    'a_m_m_hasjew_01',
    'a_m_m_hillbilly_01',
    'a_m_m_hillbilly_02',
    'a_m_m_indian_01',
    'a_m_m_ktown_01',
    'a_m_m_malibu_01',
    'a_m_m_mexcntry_01',
    'a_m_m_mexlabor_01',
    'a_m_m_og_boss_01',
    'a_m_m_paparazzi_01',
    'a_m_m_polynesian_01',
    'a_m_m_prolhost_01',
    'a_m_m_rurmeth_01',
    'a_m_m_salton_01',
    'a_m_m_salton_02',
    'a_m_m_salton_03',
    'a_m_m_salton_04',
    'a_m_m_skater_01',
    'a_m_m_skidrow_01',
    'a_m_m_socenlat_01',
    'a_m_m_soucent_01',
    'a_m_m_soucent_02',
    'a_m_m_soucent_03',
    'a_m_m_soucent_04',
    'a_m_m_stlat_02',
    'a_m_m_tennis_01',
    'a_m_m_tourist_01',
    'a_m_m_trampbeac_01',
    'a_m_m_tramp_01',
    'a_m_m_tranvest_01',
    'a_m_m_tranvest_02',
    'a_m_o_beach_01',
    'a_m_o_genstreet_01',
    'a_m_o_ktown_01',
    'a_m_o_salton_01',
    'a_m_o_soucent_01',
    'a_m_o_soucent_02',
    'a_m_o_soucent_03',
    'a_m_o_tramp_01',
    'a_m_y_beachvesp_01',
    'a_m_y_beachvesp_02',
    'a_m_y_beach_01',
    'a_m_y_beach_02',
    'a_m_y_beach_03',
    'a_m_y_bevhills_01',
    'a_m_y_bevhills_02',
    'a_m_y_breakdance_01',
    'a_m_y_busicas_01',
    'a_m_y_business_01',
    'a_m_y_business_02',
    'a_m_y_business_03',
    'a_m_y_cyclist_01',
    'a_m_y_dhill_01',
    'a_m_y_downtown_01',
    'a_m_y_eastsa_01',
    'a_m_y_eastsa_02',
    'a_m_y_epsilon_01',
    'a_m_y_epsilon_02',
    'a_m_y_gay_01',
    'a_m_y_gay_02',
    'a_m_y_genstreet_01',
    'a_m_y_genstreet_02',
    'a_m_y_golfer_01',
    'a_m_y_hasjew_01',
    'a_m_y_hiker_01',
    'a_m_y_hipster_01',
    'a_m_y_hipster_02',
    'a_m_y_hipster_03',
    'a_m_y_indian_01',
    'a_m_y_jetski_01',
    'a_m_y_juggalo_01',
    'a_m_y_ktown_01',
    'a_m_y_ktown_02',
    'a_m_y_latino_01',
    'a_m_y_methhead_01',
    'a_m_y_mexthug_01',
    'a_m_y_motox_01',
    'a_m_y_motox_02',
    'a_m_y_musclbeac_01',
    'a_m_y_musclbeac_02',
    'a_m_y_polynesian_01',
    'a_m_y_roadcyc_01',
    'a_m_y_runner_01',
    'a_m_y_runner_02',
    'a_m_y_salton_01',
    'a_m_y_skater_01',
    'a_m_y_skater_02',
    'a_m_y_soucent_01',
    'a_m_y_soucent_02',
    'a_m_y_soucent_03',
    'a_m_y_soucent_04',
    'a_m_y_stbla_01',
    'a_m_y_stbla_02',
    'a_m_y_stlat_01',
    'a_m_y_stwhi_01',
    'a_m_y_stwhi_02',
    'a_m_y_sunbathe_01',
    'a_m_y_surfer_01',
    'a_m_y_vindouche_01',
    'a_m_y_vinewood_01',
    'a_m_y_vinewood_02',
    'a_m_y_vinewood_03',
    'a_m_y_vinewood_04',
    'a_m_y_yoga_01',
    'g_m_m_armboss_01',
    'g_m_m_armgoon_01',
    'g_m_m_armlieut_01',
    'g_m_m_chemwork_01',
    'g_m_m_chiboss_01',
    'g_m_m_chicold_01',
    'g_m_m_chigoon_01',
    'g_m_m_chigoon_02',
    'g_m_m_korboss_01',
    'g_m_m_mexboss_01',
    'g_m_m_mexboss_02',
    'g_m_y_armgoon_02',
    'g_m_y_azteca_01',
    'g_m_y_ballaeast_01',
    'g_m_y_ballaorig_01',
    'g_m_y_ballasout_01',
    'g_m_y_famca_01',
    'g_m_y_famdnf_01',
    'g_m_y_famfor_01',
    'g_m_y_korean_01',
    'g_m_y_korean_02',
    'g_m_y_korlieut_01',
    'g_m_y_lost_01',
    'g_m_y_lost_02',
    'g_m_y_lost_03',
    'g_m_y_mexgang_01',
    'g_m_y_mexgoon_01',
    'g_m_y_mexgoon_02',
    'g_m_y_mexgoon_03',
    'g_m_y_pologoon_01',
    'g_m_y_pologoon_02',
    'g_m_y_salvaboss_01',
    'g_m_y_salvagoon_01',
    'g_m_y_salvagoon_02',
    'g_m_y_salvagoon_03',
    'g_m_y_strpunk_01',
    'g_m_y_strpunk_02',
    'mp_m_claude_01',
    'mp_m_exarmy_01',
    'mp_m_shopkeep_01',
    's_m_m_ammucountry',
    's_m_m_autoshop_01',
    's_m_m_autoshop_02',
    's_m_m_bouncer_01',
    's_m_m_chemsec_01',
    's_m_m_cntrybar_01',
    's_m_m_dockwork_01',
    's_m_m_doctor_01',
    's_m_m_fiboffice_01',
    's_m_m_fiboffice_02',
    's_m_m_gaffer_01',
    's_m_m_gardener_01',
    's_m_m_gentransport',
    's_m_m_hairdress_01',
    's_m_m_highsec_01',
    's_m_m_highsec_02',
    's_m_m_janitor',
    's_m_m_lathandy_01',
    's_m_m_lifeinvad_01',
    's_m_m_linecook',
    's_m_m_lsmetro_01',
    's_m_m_mariachi_01',
    's_m_m_marine_01',
    's_m_m_marine_02',
    's_m_m_migrant_01',
    's_m_m_movalien_01',
    's_m_m_movprem_01',
    's_m_m_movspace_01',
    's_m_m_pilot_01',
    's_m_m_pilot_02',
    's_m_m_postal_01',
    's_m_m_postal_02',
    's_m_m_scientist_01',
    's_m_m_security_01',
    's_m_m_strperf_01',
    's_m_m_strpreach_01',
    's_m_m_strvend_01',
    's_m_m_trucker_01',
    's_m_m_ups_01',
    's_m_m_ups_02',
    's_m_o_busker_01',
    's_m_y_airworker',
    's_m_y_ammucity_01',
    's_m_y_armymech_01',
    's_m_y_autopsy_01',
    's_m_y_barman_01',
    's_m_y_baywatch_01',
    's_m_y_blackops_01',
    's_m_y_blackops_02',
    's_m_y_busboy_01',
    's_m_y_chef_01',
    's_m_y_clown_01',
    's_m_y_construct_01',
    's_m_y_construct_02',
    's_m_y_cop_01',
    's_m_y_dealer_01',
    's_m_y_devinsec_01',
    's_m_y_dockwork_01',
    's_m_y_doorman_01',
    's_m_y_dwservice_01',
    's_m_y_dwservice_02',
    's_m_y_factory_01',
    's_m_y_garbage',
    's_m_y_grip_01',
    's_m_y_marine_01',
    's_m_y_marine_02',
    's_m_y_marine_03',
    's_m_y_mime',
    's_m_y_pestcont_01',
    's_m_y_pilot_01',
    's_m_y_prismuscl_01',
    's_m_y_prisoner_01',
    's_m_y_robber_01',
    's_m_y_shop_mask',
    's_m_y_strvend_01',
    's_m_y_uscg_01',
    's_m_y_valet_01',
    's_m_y_waiter_01',
    's_m_y_winclean_01',
    's_m_y_xmech_01',
    's_m_y_xmech_02',
    'u_m_m_aldinapoli',
    'u_m_m_bankman',
    'u_m_m_bikehire_01',
    'u_m_m_fibarchitect',
    'u_m_m_filmdirector',
    'u_m_m_glenstank_01',
    'u_m_m_griff_01',
    'u_m_m_jesus_01',
    'u_m_m_jewelsec_01',
    'u_m_m_jewelthief',
    'u_m_m_markfost',
    'u_m_m_partytarget',
    'u_m_m_prolsec_01',
    'u_m_m_promourn_01',
    'u_m_m_rivalpap',
    'u_m_m_spyactor',
    'u_m_m_willyfist',
    'u_m_o_finguru_01',
    'u_m_o_taphillbilly',
    'u_m_o_tramp_01',
    'u_m_y_abner',
    'u_m_y_antonb',
    'u_m_y_babyd',
    'u_m_y_baygor',
    'u_m_y_burgerdrug_01',
    'u_m_y_chip',
    'u_m_y_cyclist_01',
    'u_m_y_fibmugger_01',
    'u_m_y_guido_01',
    'u_m_y_gunvend_01',
    'u_m_y_imporage',
    'u_m_y_mani',
    'u_m_y_militarybum',
    'u_m_y_paparazzi',
    'u_m_y_party_01',
    'u_m_y_pogo_01',
    'u_m_y_prisoner_01',
    'u_m_y_proldriver_01',
    'u_m_y_rsranger_01',
    'u_m_y_sbike',
    'u_m_y_staggrm_01',
    'u_m_y_tattoo_01',
    'u_m_y_zombie_01',
    'u_m_y_hippie_01',
    'a_m_y_hippy_01',
    'a_m_y_stbla_m',
    'ig_terry_m',
    'a_m_m_ktown_m',
    'a_m_y_skater_m',
    'u_m_y_coop',
    'ig_car3guy1_m',
    'tony',
    'g_m_m_chigoon_02_m', ]]
}

Config.LoadedManModels = {}
Config.LoadedWomanModels = {}


Config.Stores = {
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(1685.8996582031, 4830.1655273438),
        zone = PolyZone:Create({
            vector2(1685.8996582031, 4830.1655273438),
            vector2(1687.8397216797, 4815.4614257812),
            vector2(1699.8541259766, 4817.1440429688),
            vector2(1699.4880371094, 4832.0375976562)
        }, {
            name="kiyafet1",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(-710.86645507812, -165.10945129395),
        zone = PolyZone:Create({
            vector2(-710.86645507812, -165.10945129395),
            vector2(-721.05480957031, -147.39605712891),
            vector2(-712.88800048828, -144.42813110352),
            vector2(-705.67138671875, -149.85957336426),
            vector2(-700.11590576172, -150.3238067627),
            vector2(-699.20428466797, -152.26374816895),
            vector2(-703.00360107422, -160.07594299316)
        }, {
            name="kiyafet2",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(-1200.5401611328, -784.46215820312),
        zone = PolyZone:Create({
            vector2(-1200.5401611328, -784.46215820312),
            vector2(-1201.7572021484, -782.82983398438),
            vector2(-1201.4144287109, -780.75720214844),
            vector2(-1199.4028320312, -779.26525878906),
            vector2(-1202.3199462891, -775.35479736328),
            vector2(-1204.2076416016, -777.01788330078),
            vector2(-1206.3055419922, -776.65362548828),
            vector2(-1207.6925048828, -774.86584472656),
            vector2(-1190.5494384766, -762.5751953125),
            vector2(-1187.3837890625, -761.298828125),
            vector2(-1184.9050292969, -765.91174316406),
            vector2(-1180.6319580078, -762.66363525391),
            vector2(-1179.4256591797, -764.33050537109),
            vector2(-1183.8443603516, -767.48773193359),
            vector2(-1182.3890380859, -769.48852539062)
        }, {
            name="kiyafet3",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(418.61126708984, -812.64849853516),
        zone = PolyZone:Create({
            vector2(418.61126708984, -812.64849853516),
            vector2(418.59124755859, -797.91107177734),
            vector2(431.97476196289, -797.9140625),
            vector2(431.94683837891, -804.32781982422),
            vector2(430.57855224609, -812.51507568359)
        }, {
            name="kiyafet4",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(-153.64421081543, -295.70358276367),
        zone = PolyZone:Create({
            vector2(-153.64421081543, -295.70358276367),
            vector2(-160.25930786133, -315.05709838867),
            vector2(-168.00434875488, -310.84503173828),
            vector2(-168.84007263184, -301.81616210938),
            vector2(-171.67227172852, -297.27914428711),
            vector2(-170.97648620605, -295.20126342773),
            vector2(-162.83102416992, -293.0358581543)
        }, {
            name="kiyafet5",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(82.339218139648, -1401.2438964844),
        zone = PolyZone:Create({
            vector2(82.339218139648, -1401.2438964844),
            vector2(68.465690612793, -1401.2481689453),
            vector2(69.020111083984, -1394.8198242188),
            vector2(70.234710693359, -1386.4799804688),
            vector2(82.314506530762, -1386.5581054688)
        }, {
            name="kiyafet 6",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(-813.35583496094, -1076.3732910156),
        zone = PolyZone:Create({
            vector2(-813.35583496094, -1076.3732910156),
            vector2(-826.13665771484, -1083.7574462891),
            vector2(-832.94000244141, -1072.0089111328),
            vector2(-819.51422119141, -1066.0490722656)
        }, {
            name="kiyafet7",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(-1462.1739501953, -240.29034423828),
        zone = PolyZone:Create({
            vector2(-1462.1739501953, -240.29034423828),
            vector2(-1448.5577392578, -225.15390014648),
            vector2(-1443.1811523438, -231.98936462402),
            vector2(-1446.0865478516, -240.5563659668),
            vector2(-1444.9871826172, -245.91374206543),
            vector2(-1446.36328125, -247.39823913574),
            vector2(-1454.9477539062, -246.28150939941)
        }, {
            name="kiyafet8",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(-4.6165046691895, 6513.1479492188),
        zone = PolyZone:Create({
            vector2(-4.6165046691895, 6513.1479492188),
            vector2(6.1451058387756, 6523.1865234375),
            vector2(15.45094203949, 6513.1875),
            vector2(3.6462807655334, 6504.4868164062)
        }, {
            name="kiyafet9",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(612.39031982422, 2747.03125),
        zone = PolyZone:Create({
            vector2(612.39031982422, 2747.03125),
            vector2(611.64288330078, 2769.8024902344),
            vector2(616.61975097656, 2776.0617675781),
            vector2(618.40551757812, 2776.3037109375),
            vector2(621.25915527344, 2771.0454101562),
            vector2(623.86657714844, 2747.7705078125),
            vector2(622.12359619141, 2747.5361328125),
            vector2(620.52978515625, 2748.9265136719),
            vector2(620.33831787109, 2751.3830566406),
            vector2(615.68322753906, 2751.0974121094),
            vector2(615.84069824219, 2748.58984375),
            vector2(614.40972900391, 2747.0102539062)
        }, {
            name="kiyafet10",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(1188.3782958984, 2716.5703125),
        zone = PolyZone:Create({
            vector2(1188.3782958984, 2716.5703125),
            vector2(1202.9870605469, 2715.2487792969),
            vector2(1203.3488769531, 2703.0417480469),
            vector2(1188.3149414062, 2703.2392578125)
        }, {
            name="kiyafet11",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(-3172.2329101562, 1061.6895751953),
        zone = PolyZone:Create({
            vector2(-3172.2329101562, 1061.6895751953),
            vector2(-3170.4267578125, 1060.9071044922),
            vector2(-3169.63671875, 1058.8791503906),
            vector2(-3170.6550292969, 1056.6370849609),
            vector2(-3166.3298339844, 1054.6983642578),
            vector2(-3165.3764648438, 1057.0301513672),
            vector2(-3163.3598632812, 1057.841796875),
            vector2(-3161.125, 1056.8330078125),
            vector2(-3172.5615234375, 1036.1121826172),
            vector2(-3178.787109375, 1033.0718994141),
            vector2(-3180.5993652344, 1033.8356933594),
            vector2(-3180.6806640625, 1039.7429199219)
        }, {
            name="kiyafet12",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(-1091.9829101562, 2709.6491699219),
        zone = PolyZone:Create({
            vector2(-1091.9829101562, 2709.6491699219),
            vector2(-1103.0356445312, 2699.8571777344),
            vector2(-1111.9705810547, 2709.7937011719),
            vector2(-1107.1424560547, 2713.9892578125),
            vector2(-1100.1926269531, 2718.5329589844)
        }, {
            name="kiyafet13",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(-1203.5356445312, -1459.5485839844),
        zone = PolyZone:Create({
            vector2(-1203.5356445312, -1459.5485839844),
            vector2(-1210.2648925781, -1465.7473144531),
            vector2(-1216.5046386719, -1457.0047607422),
            vector2(-1209.0111083984, -1451.7147216797)
        }, {
            name="kiyafet14",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(122.8928527832, -205.94874572754),
        zone = PolyZone:Create({
            vector2(122.8928527832, -205.94874572754),
            vector2(124.81025695801, -206.6634979248),
            vector2(125.71154785156, -208.57774353027),
            vector2(124.85503387451, -210.89176940918),
            vector2(129.23416137695, -212.50172424316),
            vector2(130.09338378906, -210.16854858398),
            vector2(132.03707885742, -209.27716064453),
            vector2(134.06098937988, -210.07257080078),
            vector2(124.37784576416, -231.36656188965),
            vector2(118.24362182617, -234.93994140625),
            vector2(116.407081604, -234.42546081543),
            vector2(115.91317749023, -228.45652770996)
        }, {
            name="kiyafet15",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(5076.8735351562, -4604.4858398438),
        zone = PolyZone:Create({
            vector2(5076.8735351562, -4604.4858398438),
            vector2(5081.1630859375, -4594.5732421875),
            vector2(5052.0888671875, -4584.0375976562),
            vector2(5048.1899414062, -4594.443359375)
        }, {
            name="kiyafet16",
        })
    },
    { 
        blip = true, 
        shopType = "clothing", 
        blipCoords = vector2(4918.1376953125, -5228.2973632812),
        zone = PolyZone:Create({
            vector2(4918.1376953125, -5228.2973632812),
            vector2(4928.9326171875, -5243.6630859375),
            vector2(4922.451171875, -5248.2612304688),
            vector2(4911.5571289062, -5232.7548828125)
        }, {
            name="kiyafet17",
        })
    },
    { -- Casino Kıyafet 1
        blip = false, 
        shopType = "clothing", 
        blipCoords = vector3(959.52716064453, -3061.8576660156, 5.9007654190063),
        zone = PolyZone:Create({
            vector2(1107.5314941406, 198.39907836914),
            vector2(1101.4641113281, 204.30551147461),
            vector2(1098.7005615234, 200.96243286133),
            vector2(1096.2591552734, 203.51174926758),
            vector2(1094.0455322266, 201.12525939941),
            vector2(1096.7465820312, 197.70643615723),
            vector2(1102.5037841797, 191.9680480957),
            vector2(1107.8044433594, 197.44059753418)
        }, {
            name = "kiyafet18",
            minZ = -50.440128326416,
            maxZ = -47.432102203369
        })
    },
    { -- unicorn
        blip = false, 
        shopType = "clothing", 
        blipCoords = vector3(959.52716064453, -3061.8576660156, 5.9007654190063),
        zone = PolyZone:Create({
            vector2(110.9681930542, -1305.9279785156),
            vector2(106.68984222412, -1308.396484375),
            vector2(102.05298614502, -1300.5065917969),
            vector2(107.52133178711, -1297.5291748047)
          }, {
            name="unicorn",
            minZ = 27.769010543823,
            maxZ = 29.76935005188
        })
    },
    { 
        blip = true, 
        shopType = "barber", 
        blipCoords = vector2(-808.67687988281, -178.70150756836),
        zone = PolyZone:Create({
            vector2(-808.67687988281, -178.70150756836),
            vector2(-807.02545166016, -181.61875915527),
            vector2(-809.30401611328, -182.96809387207),
            vector2(-808.05523681641, -185.14138793945),
            vector2(-819.75915527344, -193.23866271973),
            vector2(-825.87567138672, -182.23979187012)
        }, {
            name="berber1",
        })
    },
    { 
        blip = true, 
        shopType = "barber", 
        blipCoords = vector2(138.24328613281, -1702.7807617188),
        zone = PolyZone:Create({
            vector2(138.24328613281, -1702.7807617188),
            vector2(142.77433776855, -1706.4708251953),
            vector2(135.54319763184, -1714.9221191406),
            vector2(133.64506530762, -1713.1563720703),
            vector2(133.92269897461, -1712.0684814453),
            vector2(131.84210205078, -1710.2606201172)
        }, {
            name="berber2",
        })
    },
    { 
        blip = true, 
        shopType = "barber", 
        blipCoords = vector2(-1277.8315429688, -1120.5123291016),
        zone = PolyZone:Create({
            vector2(-1277.8315429688, -1120.5123291016),
            vector2(-1288.8289794922, -1120.5031738281),
            vector2(-1288.8167724609, -1117.9390869141),
            vector2(-1287.7214355469, -1117.4808349609),
            vector2(-1287.7330322266, -1114.7182617188),
            vector2(-1277.8419189453, -1114.7563476562)
        }, {
            name="berber3",
        })
    },
    { 
        blip = true, 
        shopType = "barber", 
        blipCoords = vector2(1932.0532226562, 3736.1662597656),
        zone = PolyZone:Create({
            vector2(1932.0532226562, 3736.1662597656),
            vector2(1937.5708007812, 3726.6799316406),
            vector2(1935.3822021484, 3725.3244628906),
            vector2(1934.3698730469, 3726.1408691406),
            vector2(1932.0354003906, 3724.7399902344),
            vector2(1926.896484375, 3733.2145996094)
        }, {
            name="berber4",
        })
    },
    { 
        blip = true, 
        shopType = "barber", 
        blipCoords =  vector2(1205.7321777344, -474.38525390625),
        zone = PolyZone:Create({
            vector2(1205.7321777344, -474.38525390625),
            vector2(1206.3671875, -471.94644165039),
            vector2(1207.5393066406, -471.73193359375),
            vector2(1208.2751464844, -469.08966064453),
            vector2(1217.8385009766, -471.66714477539),
            vector2(1216.3311767578, -477.24276733398)
        }, {
            name="berber5",
        })
    },
    { 
        blip = true, 
        shopType = "barber", 
        blipCoords = vector2(-34.027080535889, -145.63784790039),
        zone = PolyZone:Create({
            vector2(-34.027080535889, -145.63784790039),
            vector2(-31.625919342041, -146.50546264648),
            vector2(-31.540845870972, -147.6526184082),
            vector2(-28.885417938232, -148.67314147949),
            vector2(-32.37495803833, -157.97094726562),
            vector2(-37.777011871338, -155.93859863281)
        }, {
            name="berber6",
        })
    },
    { 
        blip = true, 
        shopType = "barber", 
        blipCoords = vector2(-272.98220825195, 6226.4267578125),
        zone = PolyZone:Create({
            vector2(-272.98220825195, 6226.4267578125),
            vector2(-277.04840087891, 6222.3315429688),
            vector2(-284.99057006836, 6230.1166992188),
            vector2(-283.04922485352, 6231.9130859375),
            vector2(-281.93325805664, 6231.47265625),
            vector2(-280.00537109375, 6233.3740234375)
        }, {
            name="berber7",
        })
    },
}

Config.ClothingRooms = {
    { -- LS Karakol Ust
        requiredJob = "police", 
        zone = PolyZone:Create({
            vector2(456.84655761719, -1000.3770751953),
            vector2(456.81369018555, -997.05364990234),
            vector2(458.65963745117, -995.12158203125),
            vector2(464.36108398438, -995.16778564453),
            vector2(464.220703125, -1000.3955688477)
        }, {
            name="kpolis0",
            minZ = 29.689489364624,
            maxZ = 31.921285629272
        })
    },
    { -- Paleto Karakol
        requiredJob = "police", 
        zone = PolyZone:Create({
            vector2(-439.42141723633, 5994.517578125),
            vector2(-434.77337646484, 5989.8745117188),
            vector2(-437.93237304688, 5986.6728515625),
            vector2(-442.60607910156, 5991.39453125)
        }, {
            name="kpolis2",
            minZ = 30.7161693573,
            maxZ = 32.716196060181
        })
    },
    { -- Ada
        requiredJob = "police", 
        zone = PolyZone:Create({
            vector2(4448.2094726562, -4440.2924804688),
            vector2(4420.8276367188, -4450.29296875),
            vector2(4432.8393554688, -4483.25390625),
            vector2(4460.068359375, -4473.3125)
        }, {
            name="kpolis5",
            minZ = 3.3042573928833,
            maxZ = 10.9849309921265
        })
    },
    {
        requiredJob = "ambulance", 
        zone = PolyZone:Create({
            vector2(302.04885864258, -601.19696044922),
            vector2(297.35354614258, -599.50622558594),
            vector2(299.26498413086, -594.25738525391),
            vector2(304.03060913086, -596.02789306641)
        }, {
            name="kdoktor1",
            minZ = 42.225654602051,
            maxZ = 44.284076690674
        })
    },
    {
        requiredJob = "ambulance", 
        zone = PolyZone:Create({
            vector2(-255.71717834473, 6314.8999023438),
            vector2(-250.50646972656, 6309.6928710938),
            vector2(-255.91009521484, 6304.240234375),
            vector2(-261.13879394531, 6309.5009765625)
        }, {
            name="kdoktor2",
            minZ = 31.42720413208,
            maxZ = 34.812042236328
        })
    },
    {
        requiredJob = "ambulance", 
        zone = PolyZone:Create({
            vector2(1826.3264160156, 3677.0563964844),
            vector2(1827.9801025391, 3674.1923828125),
            vector2(1824.9390869141, 3672.5502929688),
            vector2(1823.3842773438, 3675.3513183594)
        }, {
            name="kdoktor3",
            minZ = 33.27490234375,
            maxZ = 36.111618041992
        })
    },
    { -- Ada
        requiredJob = "ambulance", 
        zone = PolyZone:Create({
            vector2(4448.2094726562, -4440.2924804688),
            vector2(4420.8276367188, -4450.29296875),
            vector2(4432.8393554688, -4483.25390625),
            vector2(4460.068359375, -4473.3125)
        }, {
            name="kdoktor4",
            minZ = 3.3042573928833,
            maxZ = 10.9849309921265
        })
    },
    {
        requiredJob = "motor", 
        zone = PolyZone:Create({
            vector2(990.19909667969, -132.64074707031),
            vector2(994.42333984375, -135.1280670166),
            vector2(991.32354736328, -140.39854431152),
            vector2(987.11218261719, -137.66407775879)
        }, {
            name="kmotor1",
            minZ = 73.06135559082,
            maxZ = 75.939277648926
        })
    },
}

Config.Outfits = {
    ["motor"] = {
        ["male"] = {},
        ["female"] = {}
    },
    ["police"] = {
        ["male"] = {
            [1] = {
                outfitLabel = "Çaylak Memur",
                outfitData = {
                    ["pants"]       = { item = 139, texture = 1},  -- Pantolon
                    ["arms"]        = { item = 4, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 211, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 488, texture = 11},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 169, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = 142, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [2] = {
                outfitLabel = "Memur",
                outfitData = {
                    ["pants"]       = { item = 139, texture = 1},  -- Pantolon
                    ["arms"]        = { item = 4, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 211, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 488, texture = 12},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 169, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = 142, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [3] = {
                outfitLabel = "Kıdemli Memur",
                outfitData = {
                    ["pants"]       = { item = 139, texture = 1},  -- Pantolon
                    ["arms"]        = { item = 4, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 211, texture = 1},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 488, texture = 13},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 169, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = 142, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [4] = {
                outfitLabel = "Başpolis",
                outfitData = {
                    ["pants"]       = { item = 139, texture = 1},  -- Pantolon
                    ["arms"]        = { item = 1, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 191, texture = 2},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 489, texture = 2},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 168, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 93, texture = 0},  -- Çanta
                    ["hat"]         = { item = 142, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [5] = {
                outfitLabel = "Komiser Yard/Komiser",
                outfitData = {
                    ["pants"]       = { item = 139, texture = 1},  -- Pantolon
                    ["arms"]        = { item = 0, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 15, texture = 2},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 502, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 168, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 93, texture = 0},  -- Çanta
                    ["hat"]         = { item = 142, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [6] = {
                outfitLabel = "Catisma/Operasyon",
                outfitData = {
                    ["pants"]       = { item = 133, texture = 0},  -- Pantolon
                    ["arms"]        = { item = 17, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 15, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 79, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 370, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 168, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 93, texture = 1},  -- Çanta
                    ["hat"]         = { item = 39, texture = 0},  -- Şapka
                    ["glass"]       = { item = 24, texture = 2},  -- Gözlük
                    ["ear"]         = { item = 0, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 130, texture = 0},  -- Maske
                },
            },
            [7] = {
                outfitLabel = "SWAT",
                outfitData = {
                    ["pants"]       = { item = 133, texture = 0},  -- Pantolon
                    ["arms"]        = { item = 17, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 172, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 71, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 370, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 168, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 93, texture = 1},  -- Çanta
                    ["hat"]         = { item = 158, texture = 0},  -- Şapka
                    ["glass"]       = { item = 24, texture = 2},  -- Gözlük
                    ["ear"]         = { item = 0, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 53, texture = 0},  -- Maske
                },
            },
            [8] = {
                outfitLabel = "Tören",
                outfitData = {
                    ["pants"]       = { item = 20, texture = 0},  -- Pantolon
                    ["arms"]        = { item = 82, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 15, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 492, texture = 1},  -- Gövde
                    ["shoes"]       = { item = 21, texture = 9},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 169, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 88, texture = 0},  -- Çanta
                    ["hat"]         = { item = 46, texture = 0},  -- Şapka
                    ["glass"]       = { item = 5, texture = 1},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [9] = {
                outfitLabel = "HUDSON",
                outfitData = {
                    ["pants"]       = { item = 146, texture = 0},  -- Pantolon
                    ["arms"]        = { item = 19, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 15, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 363, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 15, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 168, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = 19, texture = 0},  -- Şapka
                    ["glass"]       = { item = 5, texture = 2},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [10] = {
                outfitLabel = "Kullanım Dışı",
                outfitData = {
                    ["pants"]       = { item = 87, texture = 2},  -- Pantolon
                    ["arms"]        = { item = 0, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 42, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 27, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 190, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 15, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 6, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 52, texture = 0},  -- Çanta
                    ["hat"]         = { item = 10, texture = 6},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = 0, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 141, texture = 0},  -- Maske
                },
            },
            [11] = {
                outfitLabel = "Kullanım Dışı",
                outfitData = {
                    ["pants"]       = { item = 35, texture = 0},  -- Pantolon
                    ["arms"]        = { item = 0, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 42, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 13, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 93, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 0, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = 10, texture = 6},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = 0, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 141, texture = 0},  -- Maske
                },
            },
            [12] = {
                outfitLabel = "Kullanım Dışı",
                outfitData = {
                    ["pants"]       = { item = 31, texture = 2},  -- Pantolon
                    ["arms"]        = { item = 17, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 15, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 16, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 219, texture = 20},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 8, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = 75, texture = 0},  -- Şapka
                    ["glass"]       = { item = 25, texture = 1},  -- Gözlük
                    ["ear"]         = { item = 0, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 17, texture = 13},  -- Maske
                },
            },
            [13] = {
                outfitLabel = "Kullanım Dışı",
                outfitData = {
                    ["pants"]       = { item = 35, texture = 0},  -- Pantolon
                    ["arms"]        = { item = 75, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 37, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 13, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 200, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 0, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 52, texture = 0},  -- Çanta
                    ["hat"]         = { item = 41, texture = 0},  -- Şapka
                    ["glass"]       = { item = 5, texture = 5},  -- Gözlük
                    ["ear"]         = { item = 0, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 141, texture = 0},  -- Maske
                },
            },
        },
        ["female"] = {
            [1] = {
                outfitLabel = "Çaylak Memur",
                outfitData = {
                    ["pants"]       = { item = 165, texture = 0},  -- Pantolon
                    ["arms"]        = { item = 7, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 249, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 569, texture = 7},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 121, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = -1, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [2] = {
                outfitLabel = "Memur",
                outfitData = {
                    ["pants"]       = { item = 165, texture = 0},  -- Pantolon
                    ["arms"]        = { item = 7, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 249, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 569, texture = 1},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 121, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = -1, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [3] = {
                outfitLabel = "Kıdemli Memur",
                outfitData = {
                    ["pants"]       = { item = 165, texture = 0},  -- Pantolon
                    ["arms"]        = { item = 7, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 249, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 569, texture = 2},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 121, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = -1, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [4] = {
                outfitLabel = "Başpolis",
                outfitData = {
                    ["pants"]       = { item = 136, texture = 1},  -- Pantolon
                    ["arms"]        = { item = 9, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 69, texture = 1},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 568, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 125, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 88, texture = 0},  -- Çanta
                    ["hat"]         = { item = -1, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [5] = {
                outfitLabel = "Komiser Yard/Komiser",
                outfitData = {
                    ["pants"]       = { item = 136, texture = 1},  -- Pantolon
                    ["arms"]        = { item = 14, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 15, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 496, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 125, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 88, texture = 0},  -- Çanta
                    ["hat"]         = { item = -1, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [6] = {
                outfitLabel = "Operasyon",
                outfitData = {
                    ["pants"]       = { item = 136, texture = 1},  -- Pantolon
                    ["arms"]        = { item = 18, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 14, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 89, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 351, texture = 3},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 125, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 88, texture = 0},  -- Çanta
                    ["hat"]         = { item = 38, texture = 0},  -- Şapka
                    ["glass"]       = { item = 26, texture = 2},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 130, texture = 0},  -- Maske
                },
            },
            [7] = {
                outfitLabel = "SWAT",
                outfitData = {
                    ["pants"]       = { item = 136, texture = 1},  -- Pantolon
                    ["arms"]        = { item = 18, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 152, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 81, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 351, texture = 3},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 0, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = 114, texture = 0},  -- Şapka
                    ["glass"]       = { item = 5, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 35, texture = 0},  -- Maske
                },
            },
            [8] = {
                outfitLabel = "Tören",
                outfitData = {
                    ["pants"]       = { item = 133, texture = 3},  -- Pantolon
                    ["arms"]        = { item = 88, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 14, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 327, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 29, texture = 1},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 121, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 88, texture = 0},  -- Çanta
                    ["hat"]         = { item = 45, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [9] = {
                outfitLabel = "HUDSON",
                outfitData = {
                    ["pants"]       = { item = 144, texture = 2},  -- Pantolon
                    ["arms"]        = { item = 20, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 14, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 384, texture = 3},  -- Gövde
                    ["shoes"]       = { item = 9, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 125, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 0, texture = 0},  -- Çanta
                    ["hat"]         = { item = 19, texture = 0},  -- Şapka
                    ["glass"]       = { item = 0, texture = 0},  -- Gözlük
                    ["ear"]         = { item = -1, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 121, texture = 0},  -- Maske
                },
            },
            [10] = {
                outfitLabel = "Kullanım Dışı",
                outfitData = {
                    ["pants"]       = { item = 34, texture = 0},  -- Pantolon
                    ["arms"]        = { item = 88, texture = 0},  -- Kollar
                    ["t-shirt"]     = { item = 2, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 14, texture = 0},  -- Zırh
                    ["torso2"]      = { item = 202, texture = 0},  -- Gövde
                    ["shoes"]       = { item = 25, texture = 0},  -- Ayakkabılar
                    ["decals"]      = { item = 0, texture = 0},  -- Çıkartmalar
                    ["accessory"]   = { item = 0, texture = 0},  -- Aksesuar
                    ["bag"]         = { item = 52, texture = 0},  -- Çanta
                    ["hat"]         = { item = 40, texture = 2},  -- Şapka
                    ["glass"]       = { item = 11, texture = 2},  -- Gözlük
                    ["ear"]         = { item = 0, texture = 0},  -- Kulak Aksesuarı
                    ["mask"]         = { item = 141, texture = 0},  -- Maske
                },
            },
        }
    },
    ["ambulance"] = {
        ["male"] = {
            [1] = {
                outfitLabel = "T-Shirt",
                outfitData = {
                    ["pants"]       = { item = 35,texture = 0},  -- Broek
                    ["arms"]        = { item = 85, texture = 0},  -- Armen
                    ["t-shirt"]     = { item = 54, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Body Vest
                    ["torso2"]      = { item = 93, texture = 1},  -- Jas / Vesten
                    ["shoes"]       = { item = 25, texture = 0},  -- Schoenen
                    ["decals"]      = { item = 0, texture = 0},  -- Decals
                    ["accessory"]   = { item = 0, texture = 0},  -- Nek / Das
                    ["bag"]         = { item = 0, texture = 0},  -- Tas
                    ["hat"]         = { item = -1, texture = -1},  -- Pet
                    ["glass"]       = { item = 0, texture = 0},  -- Bril
                    ["ear"]         = { item = 0, texture = 0},  -- Oor accessoires
                    ["mask"]        = { item = 0, texture = 0},  -- Masker
                },
            },
            [2] = {
                outfitLabel = "Ambu Lang",
                outfitData = {
                    ["pants"]       = { item = 49,texture = 1},  -- Broek
                    ["arms"]        = { item = 88, texture = 0},  -- Armen
                    ["t-shirt"]     = { item = 15, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Body Vest
                    ["torso2"]      = { item = 118, texture = 7},  -- Jas / Vesten
                    ["shoes"]       = { item = 25, texture = 0},  -- Schoenen
                    ["decals"]      = { item = 0, texture = 0},  -- Decals
                    ["accessory"]   = { item = 0, texture = 0},  -- Nek / Das
                    ["bag"]         = { item = 0, texture = 0},  -- Tas
                    ["hat"]         = { item = -1, texture = -1},  -- Pet
                    ["glass"]       = { item = 0, texture = 0},  -- Bril
                    ["ear"]         = { item = 0, texture = 0},  -- Oor accessoires
                    ["mask"]        = { item = 121, texture = 0},  -- Masker
                },
            },
        },
        ["female"] = {},
    },
    ["doctor"] = {
        ["male"] = {
            [1] = {
                outfitLabel = "Jas Voor Artsen",
                outfitData = {
                    ["pants"]       = { item = 49,texture = 0},  -- Broek
                    ["arms"]        = { item = 86, texture = 0},  -- Armen
                    ["t-shirt"]     = { item = 88, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Body Vest
                    ["torso2"]      = { item = 118, texture = 7},  -- Jas / Vesten
                    ["shoes"]       = { item = 25, texture = 0},  -- Schoenen
                    ["decals"]      = { item = 0, texture = 0},  -- Decals
                    ["accessory"]   = { item = 0, texture = 0},  -- Nek / Das
                    ["bag"]         = { item = 0, texture = 0},  -- Tas
                    ["hat"]         = { item = -1, texture = -1},  -- Pet
                    ["glass"]       = { item = 0, texture = 0},  -- Bril
                    ["ear"]         = { item = 0, texture = 0},  -- Oor accessoires
                    ["mask"]        = { item = 121, texture = 0},  -- Masker
				},
			},
			[2] = {
                outfitLabel = "OVD-G",
                outfitData = {
                    ["pants"]       = { item = 48,texture = 1},  -- Broek
                    ["arms"]        = { item = 85, texture = 0},  -- Armen
                    ["t-shirt"]     = { item = 54, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Body Vest
                    ["torso2"]      = { item = 16, texture = 1},  -- Jas / Vesten
                    ["shoes"]       = { item = 25, texture = 0},  -- Schoenen
                    ["decals"]      = { item = 0, texture = 0},  -- Decals
                    ["accessory"]   = { item = 30, texture = 0},  -- Nek / Das
                    ["bag"]         = { item = 0, texture = 0},  -- Tas
                    ["hat"]         = { item = -1, texture = -1},  -- Pet
                    ["glass"]       = { item = 0, texture = 0},  -- Bril
                    ["ear"]         = { item = 0, texture = 0},  -- Oor accessoires
                    ["mask"]        = { item = 0, texture = 0},  -- Masker
				},
			},
			[3] = {
                outfitLabel = "MMT Piloot",
                outfitData = {
                    ["pants"]       = { item = 48,texture = 2},  -- Broek
                    ["arms"]        = { item = 85, texture = 0},  -- Armen
                    ["t-shirt"]     = { item = 15, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Body Vest
                    ["torso2"]      = { item = 118, texture = 3},  -- Jas / Vesten
                    ["shoes"]       = { item = 25, texture = 0},  -- Schoenen
                    ["decals"]      = { item = 0, texture = 0},  -- Decals
                    ["accessory"]   = { item = 0, texture = 0},  -- Nek / Das
                    ["bag"]         = { item = 0, texture = 0},  -- Tas
                    ["hat"]         = { item = 79, texture = 0},  -- Pet
                    ["glass"]       = { item = 0, texture = 0},  -- Bril
                    ["ear"]         = { item = 0, texture = 0},  -- Oor accessoires
                    ["mask"]        = { item = 0, texture = 0},  -- Masker	
				},
			},		
		},		
        ["female"] = {
            [1] = {
                outfitLabel = "Vrouwelijke Outfit",
                outfitData = {
                    ["pants"]       = { item = 49,texture = 4},  -- Broek
                    ["arms"]        = { item = 85, texture = 0},  -- Armen
                    ["t-shirt"]     = { item = 19, texture = 0},  -- T Shirt
                    ["vest"]        = { item = 0, texture = 0},  -- Body Vest
                    ["torso2"]      = { item = 93, texture = 2},  -- Jas / Vesten
                    ["shoes"]       = { item = 25, texture = 0},  -- Schoenen
                    ["decals"]      = { item = 0, texture = 0},  -- Decals
                    ["accessory"]   = { item = 0, texture = 0},  -- Nek / Das
                    ["bag"]         = { item = 0, texture = 0},  -- Tas
                    ["hat"]         = { item = -1, texture = 0},  -- Pet
                    ["glass"]       = { item = 0, texture = 0},  -- Bril
                    ["ear"]         = { item = 0, texture = 0},  -- Oor accessoires
                    ["mask"]        = { item = 0, texture = 0},  -- Masker
				},
            },
        },
    },
}